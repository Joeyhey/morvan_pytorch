...
B_INIT = -0.2  # use a bad bias constant initializer   使用错误的偏压常数初始化器
...
class Net(nn.Module):
    def __init__(self, batch_normalization=False):
        ...
        # BN 层
        self.bns = []
        # 对输入数据也进行 BN，有多少 x 的输入值，momentum 是用来平滑化 batch mean and stddev 的
        self.bn_input = nn.BatchNorm1d(1, momentum=0.5)  # for input data

        for i in range(N_HIDDEN):  # build hidden layers and BN layers  建层
            # 输入层的 input_size 为1，否则为 10
            ...
            if self.do_bn:
                ...
                setattr(self, 'bn%i' % i, bn)  # 注意! pytorch 一定要你将层信息变成 class 的属性，传入数字表示不同层
                self.bns.append(bn)
    ...
    def forward(self, x):
        # 真正的实现网络
        pre_activation = [x]  # 还没经历激活
        if self.do_bn: x = self.bn_input(x)  # 判断是否要加 BN
        # 对后面每层的输入都保存在列表里
        # 加括号是为做成个列表，把每层的结果都存在列表里，为了后面画图，不画图的话这段就可以不要
        layer_input = [x]
        ...
        out = self.predict(x)
        # 不仅 return 输出，还有输入和激活之前的数据
        return out, layer_input, pre_activation


# 一个有 BN 一个没有 BN 放在列表里
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCH):
        ...
        for net, l in zip(nets, losses):
            ...
            net.train()  # 收集好信息后将 net 设置成 train 模式, 继续训练
